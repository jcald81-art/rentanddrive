 ...
 * 4. Schedule inspektlabs.ai pre-inspection reminder
...
export async function createCompleteBooking(params: BookingCreationParams): Promise<BookingResult> {
  ...
  try {
    ...
    // 4. Schedule inspektlabs.ai pre-inspection reminder (1 hour before pickup)
    ...
    await supabase.from('notifications').insert({
      ...
      message: `Please complete your inspektlabs.ai pre-rental inspection before driving. Open: ${process.env.NEXT_PUBLIC_APP_URL}/inspect/${booking.id}`,

...
function LoginContent() {
  ...
  const [error, setError] = useState<string | null>(errorParam === "link_expired" ? "Link expired. Please try again." : null);
  ...
  async function handleReset(e: React.FormEvent) {
    ...
    const { error: err } = await supabase.auth.resetPasswordForEmail(
      ...
      { redirectTo: `${baseUrl}/auth/update-password?redirect=${encodeURIComponent(redirectTo)}` }
    );
    ...
    <div className="min-h-screen bg-[#0a0a0a] text-white flex flex-col items-center justify-center px-4 py-12">
      <Link href="/" className="mb-8">
        <div className="flex items-center gap-3">
          <Image src="/logo.png" alt="RAD" width={40} height={40} className="rounded-lg" />
          <span className="font-bold text-xl tracking-tight">Rent and Drive</span>
        </div>
      </Link>
      ...
      <div className="w-full max-w-md bg-white/5 border border-white/10 rounded-2xl overflow-hidden">
        {mode !== "reset" && (
          <div className="flex border-b border-white/10">
            <button
              ...
              className={`flex-1 py-4 text-sm font-semibold transition-colors ${mode === "signin" ? "text-white border-b-2 border-[#e50914]" : "text-white/40 hover:text-white/70"}`}
            >Sign In</button>
            <button
              onClick={() => { setMode("signup"); setError(null); setSuccess(null); }}
              className={`flex-1 py-4 text-sm font-semibold transition-colors ${mode === "signup" ? "text-white border-b-2 border-[#e50914]" : "text-white/40 hover:text-white/70"}`}
        ...
        <div className="p-6 space-y-5">
          {mode === "reset" && (
            <div>
              <button onClick={() => { setMode("signin"); setError(null); setSuccess(null); }} className="text-white/40 hover:text-white text-sm flex items-center gap-1 mb-4">
          ...
          {!success && (
            <form onSubmit={mode === "signin" ? handleSignIn : mode === "signup" ? handleSignUp : handleReset} className="space-y-4">
              {mode === "signup" && (
                <div>
                  ...
                  <input type="text" value={fullName} onChange={e => setFullName(e.target.value)} required placeholder="John Smith"
                    className="w-full bg-white/5 border border-white/10 rounded-xl px-4 py-3 text-white placeholder-white/20 text-sm focus:outline-none focus:border-[#e50914] transition-colors" />
                </div>
              )}
                ...
                <div>
                  <div className="flex items-center justify-between mb-1.5">
                    ...
                    {mode === "signin" && (
                      <button type="button" onClick={() => { setMode("reset"); setError(null); setSuccess(null); }}
                        className="text-xs text-[#e50914] hover:text-[#ff3d3d] transition-colors">
                        Forgot password?
                      </button>
                    )}
                  </div>
                  <div className="relative">
                    <input
                      ...
                      className="w-full bg-white/5 border border-white/10 rounded-xl px-4 py-3 pr-12 text-white placeholder-white/20 text-sm focus:outline-none focus:border-[#e50914] transition-colors"
                    />
              ...
              <button type="submit" disabled={loading}
                className="w-full bg-[#e50914] hover:bg-[#c00810] disabled:opacity-60 disabled:cursor-not-allowed text-white font-bold py-3.5 rounded-xl transition-colors text-sm flex items-center justify-center gap-2 mt-2">
        ...
        <div className="px-6 pb-6 text-center">
          <p className="text-white/30 text-xs">
            ...
            <Link href="/terms" className="text-white/50 underline hover:text-white">Terms</Link>{" "}
            and{" "}
...
export default function LoginPage() {
  return (
    <Suspense fallback={<div className="min-h-screen bg-[#0a0a0a] flex items-center justify-center"><Spinner /></div>}>
      <LoginContent />
    </Suspense>
  );
}
